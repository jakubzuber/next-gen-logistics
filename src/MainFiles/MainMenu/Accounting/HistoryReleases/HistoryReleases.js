...
import { fetchHistoryReleases, selectHistOrders } from './historyReleasesSlice';
import LeftClickMenu from './components/LeftClickMenu';
import { fetchClients } from '../../Slices/clientsSlice';
import { fetchHistoryDetailsReleases } from './components/historyReleasesDetailsSlice';
...
const HistoryReleases = () => {
    ...
    useEffect(() => {
        dispatch(fetchHistoryReleases())
        dispatch(fetchClients())
        dispatch(fetchHistoryDetailsReleases())
    }, [dispatch]);
    ...
    const columns = useMemo(
        () => [
            {
                accessorKey: 'ID_WYDANIE_HIST',
                header: 'Id',
                size: 50,
            },
            {
                accessorKey: 'WYDANIE_ID',
                header: 'Wydanie id',
                size: 100,
            },
            {
